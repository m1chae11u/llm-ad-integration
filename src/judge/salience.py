...
{ad_info}
"""
    result = call_gemini_and_extract_json(prompt, keys=["S1", "S2", "S3", "explanation"])
...
async def judge_ad_salience_async(query, response, ad_info):
    prompt = f"""
SYSTEM  
You are an impartial evaluator.
...
{ad_info}
"""
    result = await call_gemini_api(prompt, keys=["S1", "S2", "S3", "explanation"])
